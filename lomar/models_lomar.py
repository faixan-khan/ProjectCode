...
class MaskedAutoencoderViT(nn.Module):
    ...
    def forward_encoder(self, x, window_size, num_window, mask_ratio, neigh_ratio):
        ...
        x = x + torch.gather(self.pos_embed.repeat(x.shape[0],1,1), dim=1, index = indices.unsqueeze(-1).repeat(1, 1, x.shape[-1])+1)

        # to check and confirm the position encoding
        # t2 = x
        # t1_ = torch.gather(t1, dim=1, index = indices.unsqueeze(-1).repeat(1, 1, x.shape[-1]))
        # s=0
        # o=0
        # print(t2.shape,t1_.shape)
        # for i in range(0,56):
            # o+=1
        ...
        cls_token = self.cls_token.expand(x.shape[0], -1, -1) + self.pos_embed[:, :1, :] #FAIZAN
        cls_tokens = cls_token.expand(x.shape[0], -1, -1) #FAIZAN
        ...
        # print(x.shape,'before removing the neigh prediction')
        # remiving the neighbourhood prediction
    ...
    def forward_loss(self, imgs, pred, mask_indices,num_window,ids_restore):
        ...
        # print(loss)
        # print(pred.shape, target.shape,'checking the shapes')
        # print(mask_indices.sum(), 'does thie become zero?')

        loss = (loss * mask_indices).sum() / mask_indices.sum()  # mean loss on removed patches
        # print(loss,'after all the calculations')
        return loss

    def forward(self, imgs, window_size=7, num_window=4,mask_ratio=0.8, neigh_ratio=0.05):
        pred, mask_indices, ids_restore = self.forward_encoder(imgs, window_size,num_window,mask_ratio,neigh_ratio)
