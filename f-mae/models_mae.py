...
class MaskedAutoencoderViT(nn.Module):
    ...
    def forward_encoder(self, x, window_size, num_window, mask_ratio, neigh_ratio):
        ...
        x, ids_restore,mask_indices = self.generate_window_patches(x, rand_left_locations, rand_top_locations, window_size, mask_ratio, neigh_ratio)
    ...
    def forward_decoder(self, x, window_size):
        # embed tokens
    ...
    def forward_loss(self, imgs, pred, mask, num_window, ids_restore):
        """
        imgs: [N, 3, H, W]
        pred: [N, L, p*p*3]
    ...
    def forward(self, imgs, window_size=7, num_window=4, mask_ratio=0.8, neigh_ratio=0.05):
        latent, mask_indices, ids_restore = self.forward_encoder(imgs, window_size, num_window, mask_ratio, neigh_ratio)
        pred = self.forward_decoder(latent, window_size)  # [N, L, p*p*3]
